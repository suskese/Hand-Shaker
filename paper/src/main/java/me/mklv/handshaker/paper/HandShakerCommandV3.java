...
public class HandShakerCommandV3 {
    private static final int PAGE_SIZE = 10;
    private final HandShakerPlugin plugin;
    private final ConfigManager config;

    public HandShakerCommandV3(HandShakerPlugin plugin) {
        this.plugin = plugin;
    ...
    public static void register(HandShakerPlugin plugin, Commands commands) {
        try {
            HandShakerCommandV3 handler = new HandShakerCommandV3(plugin);
            
            // Build the main command tree
            var builder = Commands.literal("handshakerv3")
            ...
            commands.register(
                    ...
                    java.util.List.of("hsv3", "hs-v3")
            );
            
            plugin.getLogger().info("✓ Registered HandShaker v3 Brigadier command (/handshakerv3)");
        } catch (Exception e) {
    ...
    private int showHelp(CommandSourceStack source) {
        source.getSender().sendMessage(Component.text("     HandShaker v7 Commands").color(NamedTextColor.GOLD).decorate(TextDecoration.BOLD));
    ...
    private int showConfiguredMods(CommandSourceStack source, int pageNum) {
        ...
        if (!(source.getSender() instanceof Player)) {
            ...
            for (int i = page.startIdx(); i < page.endIdx(); i++) {
                ...
                Component removeBtn = Component.text(" [✕]")
                    ...
                    .clickEvent(ClickEvent.runCommand(CommandVisualOperations.manageRemoveCommand("/handshakerv3", entry.getKey())));
        ...
        if (page.totalPages() > 1) {
            ...
            sendPaginationNavigation(
                source,
                page.pageNum() > 1,
                "/handshakerv3 info configured_mods " + (page.pageNum() - 1),
                page.pageNum() < page.totalPages(),
                "/handshakerv3 info configured_mods " + (page.pageNum() + 1),
                "/handshakerv3 info configured_mods <page>"
            );
        }
    ...
    private int showAllMods(CommandSourceStack source, int pageNum) {
        ...
        if (page.totalPages() > 1) {
            ...
            sendPaginationNavigation(
                source,
                page.pageNum() > 1,
                "/handshakerv3 info all_mods " + (page.pageNum() - 1),
                page.pageNum() < page.totalPages(),
                "/handshakerv3 info all_mods " + (page.pageNum() + 1),
                "/handshakerv3 info all_mods <page>"
            );
        }
        source.getSender().sendMessage(sectionFooter(title));
    ...
    private int showPlayerHistory(CommandSourceStack source, String playerName, int pageNum) {
        ...
        if (page.totalPages() > 1) {
            ...
            sendPaginationNavigation(
                source,
                page.pageNum() > 1,
                "/handshakerv3 info player " + quotedPlayer + " " + (page.pageNum() - 1),
                page.pageNum() < page.totalPages(),
                "/handshakerv3 info player " + quotedPlayer + " " + (page.pageNum() + 1),
                "/handshakerv3 info player \"" + playerName + "\" <page>"
            );
        }
        source.getSender().sendMessage(sectionFooter(title));
    ...
    private int showPlayerMods(CommandSourceStack source, String playerName, int pageNum) {
        ...
        if (totalPages > 1) {
            ...
            sendPaginationNavigation(
                source,
                pageNum > 1,
                "/handshakerv3 manage player " + quotedPlayer + " page " + (pageNum - 1),
                pageNum < totalPages,
                "/handshakerv3 manage player " + quotedPlayer + " page " + (pageNum + 1),
                "/handshakerv3 manage player \"" + target.getName() + "\" page <number>"
            );
        }
    ...
    private CompletableFuture<Suggestions> suggestConfigValues(
        ...
        List<String> suggestions = CommandSuggestionOperations.configValueSuggestions(param, config.getAvailableActions());
