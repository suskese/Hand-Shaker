...
public class ConfigMigrator {
    ...
    private static void migrateV3ToV4(File v3ConfigFile, File configYml, File dataFolder) throws IOException {
        ...
        if (!requiredMods.isEmpty()) {
            ...
            saveModsYaml(requiredFile, "required", new ArrayList<>(requiredMods), null);
        }
        ...
        if (!whitelistedMods.isEmpty()) {
            ...
            saveModsYaml(whitelistedFile, "whitelisted", new ArrayList<>(whitelistedMods), null);
        }
    ...
    private static void saveModsYaml(File file, String key, List<String> mods, Map<String, String> actionMap) throws IOException {
        ...
        if (actionMap != null) {
            // For blacklisted mods (map format)
            for (String mod : mods) {
                yaml.append("  ").append(mod).append(": ").append(actionMap.getOrDefault(mod, "kick")).append("\n");
            }
        } else {
            // For lists (required, whitelisted, ignored)
            for (String mod : mods) {
